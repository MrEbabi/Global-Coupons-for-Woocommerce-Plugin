...
function global_coupons_readme_part_1()
{
    ...
    $content .= "<b><li>Number of Orders:</li></b>This restriction is selected with #number, the chosen coupon will be activate for only the customers that have enough #number of orders. Example: 7 Orders. If a customer has 5 orders for now, customer will see this global coupon as inactive until she/he has 7 orders. <u>Regular Input: Positive Integer</u>";
    $content .= "<b><li>Amount of Orders:</li></b>This restriction is selected with #amount, the chosen coupon will be activate for only the customers that have enough #amount of orders. Example: 300 USD. If a customer has several orders with total amount of 250 USD, customer will see this global coupon as inactive until she/he has 300 USD total amount. <u>Regular Input: Positive Integer</u>";
    ...
    $content .= "<b><li>Number of Reviews:</li></b>This restriction is selected with #number, the chosen coupon will be activate for only the customers that have enough #number of reviews. Example: 5 Reviews (Product or Post Comment&Rating). If a customer has 3 reviews for now, customer will see this global coupon as inactive until she/he has 5 reviews. <u>Regular Input: Positive Integer</u>";
    $content .= "<b><li>Activate Date Interval:</li></b>You may define a global coupon that will be activated between the X and Y dates. Example: 30.06.2035-25.12.2035, then customers will see this global coupon as inactive until the starting date. <u>Regular Input: DD.MM.YYYY-DD.MM.YYYY</u>";
    $content .= "<b><li>Years of Membership:</li></b>This restriction is selected with #years, the chosen coupon will be activate for only the customers that have enough #years of membership. Example: 2 Years. If a customer has registered 3 years ago, customer will see this global coupon as active. <u>Regular Input: Positive Integer</u>";
    $content .= "</div>";
    return $content;
}
